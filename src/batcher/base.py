...
class EEGImageNetDataset(Dataset):
    def __init__(self, sample_keys, chunk_len=500, num_chunks=10, ovlp=50, population_mean=0, population_std=1, gpt_only=False, normalization=True, start_samp_pnt=-1, train=None):
        
        self.dataset_dir = "../Datasets/EEGImageNet/" # TODO: remove hardcoding, use args or receive as parameter
        leave_out_subject = 5
        granularity = "coarse"
        ...
        if train:            # train set
            chosen_data = [loaded['dataset'][i] for i in range(len(loaded['dataset'])) if loaded['dataset'][i]['subject'] != leave_out_subject]
        elif train == False: # test set
            chosen_data = [loaded['dataset'][i] for i in range(len(loaded['dataset'])) if loaded['dataset'][i]['subject'] == leave_out_subject]
        else:                # train = None
    ...
    def __getitem__(self, idx):
        ...
        return self.preprocess_sample(data, seq_len=self.num_chunks, labels=label_idx)  
